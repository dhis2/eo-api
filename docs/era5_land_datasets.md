...
**Sync** — new hours are appended incrementally as they become available.
...
!!! note "About daily totals"
    Daily precipitation values represent the **total water that fell during that calendar day**. If you sum the daily values for a month, the result should approximately match the monthly total — this is guaranteed by the way the datasets are constructed.
...
Precipitation for each individual hour in mm.

**Sync** — new hours are appended incrementally.
...
This affects the `_from_hourly` daily datasets. The standard daily datasets always use UTC boundaries.
