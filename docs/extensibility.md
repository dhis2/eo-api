...
`earthkit.data` (format-agnostic readers, GRIB and BUFR in particular) is the one exception: it is **not** part of the `[server]` extra, because it is the only dependency in this project that cannot be installed on Windows — it requires `eccodeslib` and `eckit`, which in turn require `eckitlib`, and none of those three has ever published a Windows wheel ([ecmwf/earthkit-data#1105](https://github.com/ecmwf/earthkit-data/issues/1105)). If your plugin reads GRIB, install `open-climate-service[server,grib]` and declare `earthkit-data` in your instance's dependencies as you would any other plugin-owned import.
