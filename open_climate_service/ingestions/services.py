...
def _enrich_root_zarr_json_with_pyramid(response: JSONResponse, dataset_id: str) -> JSONResponse:
    ...
    pyramid exists, injecting its native multiscales attribute makes the endpoint
    ...
    try:
        ...
        if not multiscales:
            return response
        # Inject native geozarr-toolkit dict ({"layout": [...]}) — passes validate_group
        # and is what qgis-geozarr reads via multiscales.layout[n].asset.
        if not isinstance(multiscales, dict) or not multiscales.get("layout"):
            return response
