...
class ERA5LandDailyTemperaturePlugin:
    ...
    def _fetch_month(
        self, year: int, month: int, bbox: tuple[float, float, float, float]
    ) -> xr.Dataset:
        xmin, ymin, xmax, ymax = bbox
        _, last_day = calendar.monthrange(year, month)
        cutoff = _daily_availability_cutoff()
...
class ERA5LandMonthlyPlugin:
    ...
    def __init__(self, variable: str, **_: Any) -> None:
        if variable not in _CDS_VARIABLE_NAMES:
            raise ValueError(
                f"ERA5LandMonthlyPlugin: unsupported variable {variable!r}; "
                f"expected one of {list(_CDS_VARIABLE_NAMES)}"
            )
        self.variable = variable
...
class _ERA5LandEDHBase:
    ...
    def __init__(self, variable: str) -> None:
        if variable not in _CDS_VARIABLE_NAMES:
            raise ValueError(
                f"{type(self).__name__}: unsupported variable {variable!r}; "
                f"expected one of {list(_CDS_VARIABLE_NAMES)}"
            )
        self.variable = variable
        self._cache_lock = Lock()
...
class ERA5LandEDHHourlyPlugin(_ERA5LandEDHBase):
    ...
    def _fetch_day_sync(self, period_id: str, bbox: list[float]) -> xr.Dataset:
        ...
        return self._apply_transforms(
            ds.rename({"longitude": "x", "latitude": "y", "valid_time": "t"})
        )
...
class ERA5LandEDHWithCDSFallbackPrecipDailyPlugin(_ERA5LandEDHBase):
    ...
    def _fetch_edh_daily_sync(self, period_id: str, bbox: list[float]) -> xr.Dataset:
        ...
        return ds.expand_dims({"valid_time": [ts]}).rename(
            {"longitude": "x", "latitude": "y", "valid_time": "t"}
        )
