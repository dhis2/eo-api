...
def _resolve_spatial_dims(data_array):
    ...
    raise ProcessorExecuteError(
        "Could not resolve spatial dimensions. Expected one of: (lat, lon), (latitude, longitude), or (y, x)."
    )
...
def _build_affine(x_values, y_values):
    try:
        ...
        raise ProcessorExecuteError("rasterio and numpy are required for geometry/raster alignment") from exc
...
PROCESS_METADATA = {
    ...
    "description": {"en": "Aggregate NetCDF values by GeoJSON features for each time point."},
    ...
    "inputs": {
        ...
        "aggregation": {
            ...
            "schema": {"type": "string", "enum": ["mean", "sum", "min", "max"]},
            "minOccurs": 0,
            "maxOccurs": 1,
...
class AggregationProcessor(BaseProcessor):
    ...
    def execute(self, data, outputs=None):
        try:
            ...
            raise ProcessorExecuteError("xarray, rasterio, and numpy are required for aggregation processing") from exc
        ...
        with xr.open_dataset(netcdf_path) as dataset:
            if variable not in dataset.variables:
                raise ProcessorExecuteError(f"Variable '{variable}' not found in dataset")

            data_array = dataset[variable]
            if time_dimension not in data_array.dims:
                raise ProcessorExecuteError(f"Time dimension '{time_dimension}' not found in variable '{variable}'")
            ...
            output_features = []
            time_values = [_to_serializable_time(value) for value in data_array[time_dimension].values.tolist()]
        ...
        if not bool(outputs) or "features" in outputs:
            produced_outputs = {"id": "features", "value": value}
